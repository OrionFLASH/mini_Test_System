...
```
**Описание:** Типы нагрузки, которые может создавать программа.

### 6. PROCESSOR_TYPES (Типы процессоров)
```python
PROCESSOR_TYPES = {
...
### 7. OPERATING_SYSTEMS (Операционные системы)
```python
OPERATING_SYSTEMS = {
    "macOS": "Apple macOS",
...
### 8. TEST_SETTINGS (Настройки теста)
```python
TEST_SETTINGS = {
...
### 9. COMPLEXITY_SETTINGS (Настройки сложности)
```python
COMPLEXITY_SETTINGS = {
    "hash_calculation": {
...
### 10. LOG_MESSAGES (Шаблоны сообщений)
```python
LOG_MESSAGES = {
...
### 11. TEST_CONFIGS (Предопределенные конфигурации)
```python
TEST_CONFIGS = {
    "quick": {
        "test_type": "basic",
